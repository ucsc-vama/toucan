...
class PartitioningGraph
    : public boost::adjacency_list<boost::listS, boost::vecS, boost::bidirectionalS, PartitioningGraphNodeProperty,
...
private:
  struct VertexPropertyCopier {
    ...
    VertexPropertyCopier(const PartitioningGraph &src, PartitioningGraph &dst) : src_graph(src), dst_graph(dst) {}

    template <typename VertexSrc, typename VertexDst> void operator()(const VertexSrc &v_src, VertexDst &v_dst) const {
