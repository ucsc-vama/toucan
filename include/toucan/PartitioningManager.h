...
public:
  ...
  breakDirectIOConnectionWorker(PartitioningGraph &g,
  ...
  static mlir::DenseMap<uint32_t, mlir::SmallVector<uint32_t>>
  collectGraphVectorDeclInfoToFile(const PartitioningGraph &rawGraph);

  static mlir::LogicalResult
  dumpGraphVectorDeclInfoToFile(std::string fileName,
                                const mlir::DenseMap<uint32_t, mlir::SmallVector<uint32_t>> &originalVectorElementsMap);

  void levelizeGraph(const PartitioningGraph &g, mlir::SmallVector<mlir::SmallVector<uint32_t>> &graphLevels) const;

  // mlir::LogicalResult collectAndDumpGraphVectorDeclInfoToFile(const uint32_t regionId, const PartitioningGraph &g,
  // std::string fileName);

  static mlir::LogicalResult dumpGraphToFileForMicroPartitioner(const PartitioningGraph &g,
