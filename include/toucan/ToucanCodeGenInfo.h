...
struct CGOpMetaInfo {
  ...
  uint32_t getResult() {
    switch (opName) {
      ...
      case CGToucanOPName::MemRead:
        return memRead.result;
      case CGToucanOPName::ExchangeRead:
        return exgRead.localVal;
      case CGToucanOPName::ExchangeWrite:
        return exgWrite.exchangeVal;
...
struct CGInfo {
  ...
  mlir::DenseMap<mlir::StringRef, mlir::SmallVector<std::tuple<uint32_t, uint32_t, uint32_t>>> signalDebugInfo;
