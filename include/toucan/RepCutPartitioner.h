...
public:
  ...
  RepCutPartitioner(std::filesystem::path outputDirectory, std::shared_ptr<PartitioningGraph> graph)
      : outputDirectory(outputDirectory), _graph(graph) {
...
private:
  ...
  mlir::LogicalResult callRepCutAndWait(uint32_t nParts, float target_ib, const std::string &graphFile,
                                        const std::filesystem::path &workingDirectory, int maxThreads);

  static mlir::LogicalResult parseRepCutResult(uint32_t nParts, const std::string &resultFile,
  ...
  mlir::LogicalResult workerFunc(const PartitioningGraph &graph, std::filesystem::path workDirectory,
                                 mlir::SmallVector<mlir::SmallVector<uint32_t>> &partOutput, uint32_t nParts,
                                 int maxThreads);

  bool keepRepartitionMayUseless;
  mlir::LogicalResult rePartition(mlir::MLIRContext *context, const PartitioningGraph &graph,
                                  std::filesystem::path regionWorkDirectory,
                                  mlir::SmallVector<mlir::SmallVector<uint32_t>> &partOutput, const uint32_t iterId,
                                  uint32_t &previousRePartitionInputNumParts);
