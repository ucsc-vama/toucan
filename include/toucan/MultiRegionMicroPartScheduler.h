...
//   void generateRegMemLayout(DesignGraph &graph, uint32_t partitionRegPaddingSpace = 32, uint32_t memPaddingSpace =
//   32);
...
//   void schedule(mlir::MLIRContext *context, const PartitioningGraph &graph, const
//   mlir::SmallVector<mlir::SmallVector<uint32_t>> &partNodeLis); void generateRegMemLayout(const PartitioningGraph
//   &graph, const mlir::SmallVector<mlir::SmallVector<uint32_t>> &partNodeList);
...
//   void sortRegistersForLocality(const PartitioningGraph &graph, const mlir::SmallVector<mlir::SmallVector<uint32_t>>
//   &partNodeList, mlir::SmallVector<mlir::SmallVector<mlir::TypedValue<toucan::RegType>>> &regOrdered); void
//   fillRegPool(mlir::SmallVector<mlir::SmallVector<mlir::TypedValue<toucan::RegType>>> regPoolOrdered); void
//   fillMemPool(const PartitioningGraph &graph); void sortRegWriteOps(const PartitioningGraph &graph,
//   mlir::SmallVector<uint32_t> &allRegWrites) const; void sortRegReadOps(const PartitioningGraph &graph,
//   mlir::SmallVector<uint32_t> &allRegReads) const;

//   void collectConstantVars(const PartitioningGraph &graph);
//   void collectConstantVecs(const PartitioningGraph &graph, CGPartitionMetaInfo &partInfo, const
//   mlir::SmallVector<uint32_t> &partNodes);

//   void scheduleRegReads(const PartitioningGraph &graph, CGPartitionMetaInfo &partInfo, const
//   mlir::SmallVector<uint32_t> &allRegReads);

//   void scheduleRegWrites(const PartitioningGraph &graph, CGPartitionMetaInfo &partInfo, const
//   mlir::SmallVector<uint32_t> &allRegWrites); void scheduleMemWrites(const PartitioningGraph &graph,
//   CGPartitionMetaInfo &partInfo, const mlir::SmallVector<uint32_t> &allMemWrites); void scheduleStops(const
//   PartitioningGraph &graph, CGPartitionMetaInfo &partInfo, const mlir::SmallVector<uint32_t> &allStops); void
//   schedulePrints(const PartitioningGraph &graph, CGPartitionMetaInfo &partInfo, const mlir::SmallVector<uint32_t>
//   &allPrints);

//   void scheduleMicroParts(const PartitioningGraph &graph, CGPartitionMetaInfo &partInfo, const
...
public:
  ...
  mlir::LogicalResult schedule(mlir::MLIRContext *context, const PartitioningGraph &rawGraph,
                               const mlir::SmallVector<mlir::Value> &exchangeValPool);
  void buildDummyVtxIndexInVec(const MicroPartitioner &mPartitioner);
  void takeVecTables(MicroPartitioner &mPartitioner);
...
private:
  ...
  void scheduleRegularMicroPart(const PartitioningGraph &graph, CGMicroPartInfo &part, const MicroPart &mPart,
                                const mlir::DenseMap<mlir::Value, uint32_t> &valToValId) const;
  void scheduleSpecialMicroPart(const PartitioningGraph &graph, CGMicroPartInfo &part, const MicroPart &mPart,
                                const CGPartitionMetaInfo &partInfo) const;
  void scheduleNOPMicroPart(CGMicroPartInfo &part, MicroPart &mPart,
                            const mlir::DenseMap<mlir::Value, uint32_t> &valToValId) const;
  void scheduleMicroParts(const PartitioningGraph &graph, CGPartitionMetaInfo &partInfo,
                          const std::vector<std::vector<MicroPart>> &partLevels);
