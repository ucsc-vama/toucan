...
struct EnsureNoClockOpPass : toucan::impl::EnsureNoClockOpBase<EnsureNoClockOpPass> {
  ...
  LogicalResult runOnModule(hw::HWModuleOp mod) {
    ...
    for (auto &stmt : mod.getOps()) {
      if (isa<seq::ToClockOp>(stmt) || isa<seq::FromClockOp>(stmt) || isa<seq::ConstClockOp>(stmt) ||
          isa<seq::ClockGateOp>(stmt) || isa<seq::ClockDivider>(stmt) || isa<seq::ClockMuxOp>(stmt)) {
...
std::unique_ptr<mlir::Pass> toucan::createEnsureNoClockOpPass() { return std::make_unique<EnsureNoClockOpPass>(); }
