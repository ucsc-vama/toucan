...
namespace toucan {
StringRef getSVNameHitRef() { return "sv.namehint"; }
...
void setSVNameHintAttr(Operation *op, StringAttr &value) { op->setAttr(getSVNameHitRef(), value); }
...
bool hasSignalFragmentId(mlir::Operation *op) { return op->hasAttr(getSignalFragmentIDRef()); }
...
const char *getRegNextSuffix() { return "$next"; }

StringRef getSignalFragmentIDRef() { return "signal_fragment_id"; }

void setSignalFragmentIDAttr(Operation *op, IntegerAttr &id) { op->setAttr(getSignalFragmentIDRef(), id); }
...
StringRef getAccumulatedMemWidthAttr() { return "accumulated_mem_width"; }

void setAccumulatedMemWidthAttr(Operation *op, IntegerAttr &id) { op->setAttr(getAccumulatedMemWidthAttr(), id); }
...
mlir::StringRef getIOSignalMarkerAttrName() { return "resultIsIO"; }
...
void removeIOSignalMarker(mlir::Operation *op) { op->removeAttr(getIOSignalMarkerAttrName()); }

mlir::StringRef getMulIDAttrName() { return "MulID"; }
...
IntegerAttr getMulIdAttr(Operation *op) { return op->getAttrOfType<IntegerAttr>(getMulIDAttrName()); }

void setMulId(Operation *op, IntegerAttr id) { op->setAttr(getMulIDAttrName(), id); }

mlir::StringRef getAddIDAttrName() { return "AddID"; }
...
void setAddId(Operation *op, IntegerAttr id) { op->setAttr(getAddIDAttrName(), id); }
...
llvm::SmallVector<mlir::Value> split_value_4B(mlir::Operation *op, const mlir::Value &value,
...
void concat_4b_and_replace(mlir::Operation *op, mlir::Value opResult, llvm::SmallVector<mlir::Value> &values,
                           mlir::RewriterBase &rewriter) {
  if (values.size() > 1) {
...
void attachNameHintAndFragmentId(RewriterBase &rewriter, mlir::SmallVector<mlir::Value> &values,
                                 std::optional<mlir::StringAttr> namehint) {
  if (namehint) {
...
void attachNameHintAndFragmentId(mlir::RewriterBase &rewriter, mlir::Value &value,
                                 std::optional<mlir::StringAttr> namehint) {
  if (namehint) {
    auto valDefiningOp = value.getDefiningOp();
...
void attachNameHintAndFragmentId(mlir::RewriterBase &rewriter, mlir::Operation *op,
                                 std::optional<mlir::StringAttr> namehint) {
  if (namehint) {
    auto valFragmentId = rewriter.getI32IntegerAttr(0);
...
mlir::Value generate_mux_chain_for_array(mlir::Operation *op, mlir::RewriterBase &rewriter,
                                         llvm::SmallVector<mlir::Value> values, mlir::Value index) {
...
mlir::Value
generate_reduce_tree(mlir::RewriterBase &rewritter, Location loc, llvm::SmallVector<mlir::Value> &inputs,
                     std::function<mlir::Value(mlir::RewriterBase &, mlir::Location, mlir::Value, mlir::Value)> cb) {
...
bool opHasResultValue(mlir::Operation *op) { llvm_unreachable("Wait later"); }
